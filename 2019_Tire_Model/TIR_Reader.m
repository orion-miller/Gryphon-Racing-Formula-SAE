...
[FileName, FilePath] = uigetfile({'*.tir','Tire Model File'},'Select TIR File to Import');
fileID = fopen([FilePath FileName],'r'); % Open file filename for reading ('r')
...
for ii = 1:length(MFParamName)
    if cellfun(@length,MFParamName(ii)) > 1
        if any(isstrprop(MFParamValue{ii},'digit')) > 0
            TIR.(MFParamName{ii}) = str2double(MFParamValue{ii});
        else
            TIR.(MFParamName{ii}) = MFParamValue{ii};
        end
    end
end
...
for jj = 1:length(MFParamName)
    if cellfun(@length,MFParamName(jj)) > 1
        TIR.TIRDescription.(MFParamName{jj}) = MFParamDescription{jj};
    end
end
